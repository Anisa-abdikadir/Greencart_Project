...
const Cart = () => {
    ...
    const {products,currency , cartItems,removeFormCart,getCartCount, 
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.addresses.length>0) {
                    setSlectedAddress(data.addresses[0])
        ...
        <div className="flex flex-col md:flex-row  mt-16">
            <div className='flex-1 max-w-4xl'>
                ...
                {CartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div onClick={() => {navigate(`/products/${product.category.toLowerCase()}/${product._id}`); scroll(0,0)}}
                        ...
                        <button onClick={() =>removeFormCart(product._id)}
                ...
                <button onClick={()=>{navigate('/products');scrollTo(0, 0);}} className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                    <img className="group-hover:-translate-x-1 transition-transform" src={assets.arrow_right_icon_colored} alt="arrow left"  />
                    Continue Shopping
                </button>
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                               {addresses.map((address, index) => (
                                <p onClick={() =>{setSlectedAddress(address); setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
